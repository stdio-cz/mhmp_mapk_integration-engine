...
import "mocha";
import { WazeCCPAlertsTransformation } from "../../../src/modules/wazeccp";
...
describe("WazeCCPAlertsTransformation", () => {
    ...
    before(() => {
        validator = new Validator(WazeCCP.alerts.name + "ModelValidator",
            WazeCCP.alerts.outputMongooseSchemaObject);
    });

    beforeEach(async () => {
        transformation = new WazeCCPAlertsTransformation();
        const buffer = await readFile(__dirname + "/../../data/wazeccp_alerts-datasource.json");
        testSourceData = JSON.parse(Buffer.from(buffer).toString("utf8"));
    });

    it("should has name", async () => {
        expect(transformation.name).not.to.be.undefined;
        expect(transformation.name).is.equal("WazeCCPAlerts");
    });
