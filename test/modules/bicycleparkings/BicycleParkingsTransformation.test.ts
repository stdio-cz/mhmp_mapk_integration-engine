...
import "mocha";
import { BicycleParkingsTransformation } from "../../../src/modules/bicycleparkings";
...
describe("BicycleParkingsTransformation", () => {

    let transformation;
    let testSourceData;

    beforeEach(async () => {
        transformation = new BicycleParkingsTransformation();
        const buffer = await readFile(__dirname + "/../../data/bicycleparkings-datasource.json");
        testSourceData = JSON.parse(Buffer.from(buffer).toString("utf8"));
    });

    it("should has name", async () => {
        expect(transformation.name).not.to.be.undefined;
        expect(transformation.name).is.equal("BicycleParkings");
    });
