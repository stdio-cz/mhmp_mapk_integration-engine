...
import POTEXSortedWasteStationsTransformation from "../../src/transformations/POTEXSortedWasteStationsTransformation";
...
describe("POTEXSortedWasteStationsTransformation", () => {

    let transformation;
    let testSourceData;

    beforeEach(async () => {
        transformation = new POTEXSortedWasteStationsTransformation();
        const buffer = await readFile(__dirname + "/../data/potex_sortedwastestations-datasource.json");
        testSourceData = JSON.parse(Buffer.from(buffer).toString("utf8"));
    });

    it("should has name", async () => {
        expect(transformation.name).not.to.be.undefined;
        expect(transformation.name).is.equal("POTEXSortedWasteStations");
    });
