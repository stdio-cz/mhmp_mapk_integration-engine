...
import CeskyCarsharingTransformation from "../../src/transformations/CeskyCarsharingTransformation";
...
describe("CeskyCarsharingTransformation", () => {

    let transformation;
    let testSourceData;

    beforeEach(async () => {
        transformation = new CeskyCarsharingTransformation();
        const buffer = await readFile(__dirname + "/../data/ceskycarsharing-datasource.json");
        testSourceData = JSON.parse(Buffer.from(buffer).toString("utf8"));
    });

    it("should has name", async () => {
        expect(transformation.name).not.to.be.undefined;
        expect(transformation.name).is.equal("CeskyCarsharingSharedCars");
    });
    ...
    it("should properly transform element", async () => {
        const data = await transformation.transform(testSourceData.cars[0]);
    ...
    it("should properly transform collection", async () => {
        const data = await transformation.transform(testSourceData.cars);
